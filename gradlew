```bash
     ...
     #   set -- "${ARGS[@]}" "$@"
     #
     # but that's not POSIX-compliant (readarray and arrays are Bash extensions).
     # So instead, we pass each arg (as a POSIX-quoted string) via a temporary file
     # and append it to a command string that is later eval'd to re-set the arguments.

     # Set up a temporary file for POSIX-compliant argument passing.
